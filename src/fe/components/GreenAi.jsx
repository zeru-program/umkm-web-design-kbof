...
const PopupAi = ({aiActive, setAiActive}) => {
  ...
  const handleSubmit = async (e) => {
    ...
    // update chat history is user send
    setChatHistory(history => [...history, { role: "user", text: userMess }])
    
    // update res temporary chatbot
    ...
    try {
      setTimeout(async() => {
        const res = await requestAi([...chatHistory, { role: "user", text: `By using the details given above and only using Indonesian please answer this question: ${userMess}` }])
        
        setDisButton(false)
        setChatHistory(prev => [...prev.filter(msg => msg.text !== "Thinking.."), {role: "model", hideInChat: false, text: res, isError: false}])
      }, 2000);
      // console.log(res)
    } catch (error) {
      setChatHistory(prev => [...prev.filter(msg => msg.text !== "Thinking.."), {role: "model", hideInChat: false, text: error.message, isError: true}])
      // console.error(error.message, "error 2 bangss")
    }
  }
  ...
  return (
    <div className='px-3 py-2 bg-light rounded-1 popup-ai shadow-sm flex-column align-items-center position-fixed' style={{display: aiActive ? "flex" : "none",right: "20px", bottom: "140px", zIndex: "2000"}}>
      ...
      <div className='w-100 d-flex gap-2 body-chat-greenai py-3 flex-column' ref={chatBodyRef}>
        {
          ...
          chatHistory.map((chat, index) => (
            !chat.hideInChat && (
            <div key={index} className={`bubble bubble-${chat.role === "model" ? "ai" : "user"} ${chat.isError ? "text-danger" : ""} rounded-2`}>
              <span className={`sender sender-${chat.role === "model" ? "ai" : "user"}`}>{chat.role === "model" ? "GreenAi" : "User"}</span>
              <p className='mess-greenai'>{chat.text}</p>
            </div>
            )
          )) : (<div className='w-100 h-100 d-flex justify-content-center align-items-center'><span className='text-satoshi text-muted border-bottom'>Let's Talk With GreenAi !</span></div>)
        }
      </div>
      <form className='w-100 gap-2 d-flex align-items-center' onSubmit={handleSubmit}      
      >
        <div className='w-100'>
          <input type="text" ref={inputRef} placeholder='Ask Green Ai..' required className='w-100 form-control' />
        </div>
        <div>
